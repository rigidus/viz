...
#+BEGIN_SRC lisp
  ...
  (defstruct tetris-piece-s
    (origin_x 0 :type '(unsigned-byte 16))
    (origin_y 0 :type '(unsigned-byte 16))
    (x 0 :type '(unsigned-byte 16))
    (y 0 :type '(unsigned-byte 16))
    (color 0 :type '(unsigned-byte 16))
    (symmetry 0 :type '(unsigned-byte 16))
    (orientation 0 :type '(unsigned-byte 16))
    ;; (data) ;; pointer
    ;; (empty_cell) ;; 3 chars
    )
    ...
    (let* ((dims '(7 4))
           (piece-data
             (make-array
              dims
              :element-type 'fixnum
              :initial-contents '((#x1256 0 0 0)                    ;; square
                                  (#x159d #x4567 0 0)               ;; line
                                  (#x4512 #x0459 0 0)               ;; s
                                  (#x0156 #x1548 0 0)               ;; z
                                  (#x159a #x8456 #x0159 #x2654)     ;; l
                                  (#x1598 #x0456 #x2159 #xa654)     ;; r
                                  (#x1456 #x1596 #x4569 #x4159))))  ;; t
           (piece-data-len (array-dimension piece-data 0))
           (colors (make-array 7 :initial-contents
                               `(,+RED+ ,+GREEN+ ,+YELLOW+ ,+BLUE+
                                        ,+FUCHSIA+ ,+CYAN+ ,+WHITE+)))
           (next_piece_index (random piece-data-len))
           (next_piece_data
             (make-array 4
                         :displaced-to piece-data
                         :element-type 'fixnum
                         :displaced-index-offset (* (cadr dims) next_piece_index)))
           )
  ...
  (multiple-value-bind (num cnt)
      (get-next-piece 1)
    (format nil "~A | ~{~X ~}" num (array-to-list cnt)))
  ...
  ;;     (if visible
  ;;         (progn
  ;;           (set-fg (getf piece :color))
  ;;           (set-bg (getf piece :color))))
  ;;     (loop for idx from 0 to 3 :do
