...
#+BEGIN_SRC lisp
  ...
  (defun fill-show (coords)
    (loop for (x y) in coords do
      (xyprint x y +FILLED-CELL+)))

  (defun get-cells (piece position)
    "piece : struct tetpi
     position : (simple-vector 3)"
    (let* ((data (aref (tetpi-data piece) (tetpi-orientation piece)))
           (xx (tetpi-x piece))
           (yy (tetpi-y piece))
           (cells (make-array 8 :element-type 'integer :initial-element 0)))
      (when position
      ...
      (loop for idx from 0 below 4 do
        (setf (aref cells (* 2 idx))
              (logand 3 (ash data (* -4 idx))))
        (setf (aref cells (+ 1 (* 2 idx)))
              (logand 3 (ash data (- (+ 2 (* 4 idx)))))))
      cells))
  ...
  ;;   (let ((cells (cells (aref (tetpi-data piece) (tetpi-orientation piece)))))
  ;;     cells))

  ;; (let ((next-piece (get-random-piece))
  ;;       (playfield (loop for idx from 0 to +PLAYFIELD-H+ collect 0)))
  ;;   (flatten-piece next-piece playfield ))

  ;;   (draw-piece next-piece #'fill-show)))

  ;;     (loop for idx from 0 to 4 do
  ;;       (let ((x))))))


  ;; (let* ((playfield (loop for idx from 0 to +PLAYFIELD-H+ collect 0)))
  ;;   playfield)
  ...
  (defun get-piece (&key color-idx piece-idx)
    (let* ((next-piece-data
           ...
           (next_piece (make-tetpi
                        ...
                        :x 0
                        :y 0
                        :color (aref *colors* color-idx)
                        :data next-piece-data
                        :symmetry (aref next-piece-data 0)
                        ;; :orientation 0 ;; skipped for init in get-random-piece
                        :empty-cell +NEXT-EMPTY-CELL+)))
      next_piece))

  (defun get-random-piece ()
    (let ((piece (get-piece :color-idx (random (length *colors*))
                            :piece-idx (random (car *pieces-dim*)))))
      (let ((orient-idx (+ 1 (random (tetpi-symmetry piece)))))
        (setf (tetpi-orientation piece) orient-idx)
        piece)))

  (defun get-next-piece (visible)
    (let ((random-piece (get-random-piece)))
      (draw-piece random-piece visible)))
  ...
  (defun get-current-piece (next-piece playfield)
    (let ((cur (copy-structure next-piece)))
      ...
      (setf (tetpi-empty-cell cur) +PLAYFIELD-EMPTY-CELL+)
  ...
  (defun update-score (complete-lines)
    ...
    (set-bold)
    (set-fg +SCORE-COLOR+)
    (xyprint 2 0 (format nil "Lines completed: ~A   " *lines-completed*))
    (xyprint 3 0 (format nil "Level:           ~A   " *level*))
    (xyprint 4 0 (format nil "Score:           ~A   " *score*))
    (reset-colors))
  ...
  (defun draw-help (&key visible)
    (when visible
      (set-fg +HELP-COLOR+)
      (set-bold))
    (xyprint   1 58 "  Use cursor keys")
    (xyprint   2 58 "       or")
    (xyprint   3 58 "    s: rotate")
    (xyprint   4 58 "a: left  d: right")
    (xyprint   5 58 "    space: drop")
    (xyprint   6 58 "      q: quit")
    (xyprint   7 58 "  c: toggle color")
    (xyprint   8 58 "n: toggle show next")
    (xyprint   9 58 "h: toggle this help"))
  ...
  (defun redraw-screen (next-piece current-piece playfield)
    ...
    ;; (pip (draw-piece current-piece #'fill-show))
    )

  ;; init data
  (defparameter *playfield* (make-array +PLAYFIELD-H+ :initial-element 0))
  (defparameter *next-piece* (get-random-piece))
  ...
  (let* ((next-piece (get-next-piece)))
    ;; (current-piece
    (get-current-piece (get-next-piece) *playfield*))

  (redraw-screen *next-piece* *current-piece* *playfield*)
  ;; (pip (draw-piece current-piece #'fill-show))
  ;; (pip (draw-piece next-piece #'fill-show))
  )
  ...
  (defun draw-piece (piece visible)
    (let ((cells (get-cells piece nil)))
      ...
      (loop for idx from 0 below 4 do
        (let ((xx (+ (aref cells (* 2 idx))        (tetpi-origin-x piece)))
  ...
  (defun cls (sym from-x to-x from-y to-y)
    (loop for yy from from-y to to-y do
      (loop for xx from from-x to to-x do
        (xyprint yy xx (format nil "~A" sym)))))


  ;; (macroexpand-1 '(with-pipe my-pipe (some1) (some2)))
