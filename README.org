...
#+BEGIN_SRC lisp
  ...
  (defparameter *pieces-dim* `(,(length +PIESES+) ,(length (car +PIESES+))))

  (defparameter *pieses*
    (make-array *pieces-dim*
                :element-type 'fixnum
                :initial-contents +PIESES+))
  ...
  (defun draw-piece (piece)
    ...
    (net-show (cells (aref (tetpi-data piece) 2)))
    ;; (reset-colors)
    )

  ;; start
  (draw-piece (get-next-piece))
