...
#+BEGIN_SRC lisp
  ...
  (defun fill-show (coords)
    (loop for (x y) in coords do
      (xyprint (+ 10 x) (+ 10 y) +FILLED_CELL+)))

  (defun cells (data)
    "for every digit in data (like #x1256) x=((cell & 3)) y=((cell >> 2))"
    (loop for idx from 0 below 4
          collect (let* ((x-shift (- (* 4 idx)))
                         (x-seg (logand 3 (ash data x-shift)))
                         (y-shift (- (+ 2 (* 4 idx))))
                         (y-seg (logand 3 (ash data y-shift))))
                    (list x-seg y-seg))))
  ;; (cells #x1256)

  (defun offset (x y data)
    (loop for (xx yy) in data :collect (list (+ x xx) (+ y yy))))
  ;; (offset 10 20 (cells #x1256))
  ...
  ;; x=((cell & 3)) y=((cell >> 2))
  (defconstant +PIECES+ '((1 #x1256 0 0 0)
                          (2 #x159d #x4567 0 0)
                          (2 #x4512 #x0459 0 0)
                          (2 #x0156 #x1548 0 0)
                          (4 #x159a #x8456 #x0159 #x2654)
                          (4 #x1598 #x0456 #x2159 #xa654)
                          (4 #x1456 #x1596 #x4569 #x4159)))
  ...
  (defun draw-piece (piece fn)
    (funcall fn (cells (aref (tetpi-data piece) (tetpi-orientation piece)))))
  ...
  (defmacro pip (&body body)
    `(with-pipe my-pipe
       (let ((res (with-output-to-string (*standard-output*)
                    ,@body)))
  ...
  (pip
    ...
    (set-bg 1)
    ;; (xyprint 0 0 "[=..................=]")
    (xyprint 9  6 "qwertyuiop2")
    (xyprint 10 6 "qwertyuiop2")
    (xyprint 11 6 "qwertyuiop0")
    (xyprint 12 6 "qwertyuiop1")
    (xyprint 13 6 "qwertyuiop1")
    (set-fg 1)
    (set-bg 7)
    (let ((piece (get-random-piece)))
      (draw-piece piece
                  ;; #'net-show
                  #'fill-show
                  ))
